// /_/src/Flecs.NET/Generated/Entity/Entity.ComponentCallbacks/T1.g.cs
...
public unsafe partial struct Entity
{
    ...
    {
        return Invoker<T0, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _>.Read<ReadRefCallbackDelegate<T0>>(new Entity(World, Id), callback);
    }
    ...
    {
        return Invoker<T0, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _>.Write<WriteRefCallbackDelegate<T0>>(new Entity(World, Id), callback);
    }
    ...
    {
        Invoker<T0, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _>.Insert<InsertRefCallbackDelegate<T0>>(new Entity(World, Id), callback);
        return ref this;
    }
}