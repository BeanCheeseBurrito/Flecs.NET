// /_/src/Flecs.NET/Generated/Entity/Entity.ComponentCallbacks/T2.g.cs
...
{
    /// <summary>
    ///     Read 2 components using the provided callback. <br/><br/>
    /// 
    ...
    public bool Read<T0, T1>(Ecs.ReadRefCallback<T0, T1> callback)
    {
        return Invoker<T0, T1, _, _, _, _, _, _, _, _, _, _, _, _, _, _>.Read<ReadRefCallbackDelegate<T0, T1>>(new Entity(World, Id), callback);
    }

    /// <summary>
    ///     Write 2 components using the provided callback. <br/><br/>
    /// 
    ///     This operation accepts a callback with as arguments the components to
    ///     retrieve. The callback will only be invoked when the entity has all
    ///     the components. <br/><br/>
    ...
    /// <typeparam name="T0">The T0 component type.</typeparam> <typeparam name="T1">The T1 component type.</typeparam>
    /// <returns>True if the entity has the specified components.</returns>
    public bool Write<T0, T1>(Ecs.WriteRefCallback<T0, T1> callback)
    {
        return Invoker<T0, T1, _, _, _, _, _, _, _, _, _, _, _, _, _, _>.Write<WriteRefCallbackDelegate<T0, T1>>(new Entity(World, Id), callback);
    }

    /// <summary>
    ///     Ensures 2 components using the provided callback.<br/><br/>
    /// 
    ///     This operation accepts a callback with as arguments the components to
    ...
    public ref Entity Insert<T0, T1>(Ecs.InsertRefCallback<T0, T1> callback)
    {
        Invoker<T0, T1, _, _, _, _, _, _, _, _, _, _, _, _, _, _>.Insert<InsertRefCallbackDelegate<T0, T1>>(new Entity(World, Id), callback);
        return ref this;
    }
}