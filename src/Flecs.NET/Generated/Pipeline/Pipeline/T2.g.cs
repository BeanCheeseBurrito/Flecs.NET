// /_/src/Flecs.NET/Generated/Pipeline/Pipeline/T2.g.cs
...
/// <summary>
///     A type-safe wrapper around <see cref="Pipeline"/> that takes 2 type arguments.
/// </summary>
/// <typeparam name="T0">The T0 component type.</typeparam> <typeparam name="T1">The T1 component type.</typeparam>
public unsafe partial struct Pipeline<T0, T1> : IEquatable<Pipeline<T0, T1>>, IEntity<Pipeline<T0, T1>>
{
    ...
    public static implicit operator ulong(Pipeline<T0, T1> pipeline)
    {
        return ToUInt64(pipeline);
    }

    /// <inheritdoc cref="Pipeline.ToId(Pipeline)"/>
    public static implicit operator Id(Pipeline<T0, T1> pipeline)
    {
        return ToId(pipeline);
    }

    /// <inheritdoc cref="Pipeline.ToEntity(Pipeline)"/>
    public static implicit operator Entity(Pipeline<T0, T1> pipeline)
    {
        return ToEntity(pipeline);
    }

    /// <inheritdoc cref="Pipeline.ToUInt64(Pipeline)"/>
    public static ulong ToUInt64(Pipeline<T0, T1> pipeline)
    {
        return pipeline.Entity;
    }

    /// <inheritdoc cref="Pipeline.ToId(Pipeline)"/>
    public static Id ToId(Pipeline<T0, T1> pipeline)
    {
        return pipeline.Id;
    }

    /// <inheritdoc cref="Pipeline.ToEntity(Pipeline)"/>
    public static Entity ToEntity(Pipeline<T0, T1> pipeline)
    {
        return pipeline.Entity;
    }
    ...
    public bool Equals(Pipeline<T0, T1> other)
    {
    ...
    {
        return obj is Pipeline<T0, T1> other && Equals(other);
    }
    ...
    public static bool operator ==(Pipeline<T0, T1> left, Pipeline<T0, T1> right)
    {
