// /_/src/Flecs.NET/Generated/Invoker/FetchComponents/T1.g.cs
...
public static unsafe partial class Invoker
{
    ...
    public static bool Read<T0>(ecs_world_t* world, ulong entity, Ecs.ReadRefCallback<T0> callback)
    {
        ...
        void** pointers = stackalloc void*[1];
        bool hasComponents = Ecs.GetPointers<T0>(world, entity, record, table, pointers);

        if (hasComponents)
            callback(ref Managed.GetTypeRef<T0>(pointers[0]));
    ...
    /// <typeparam name="T0">The T0 component type.</typeparam>
    /// <returns>True if the entity has the specified components.</returns>
    public static bool Write<T0>(ecs_world_t* world, ulong entity, Ecs.WriteRefCallback<T0> callback)
    {
    ...
    /// <typeparam name="T0">The T0 component type.</typeparam>
    /// <returns></returns>
    public static bool Insert<T0>(World world, ulong entity, Ecs.InsertRefCallback<T0> callback)
    {
        ...
        if (!world.IsDeferred())
        {
            ...
            ulong* added = stackalloc ulong[1];
            int addedCount = 0;
            ...
            if (!Ecs.GetPointers<T0>(world, entity, record, table, pointers))
        ...
        {
            Ecs.EnsurePointers<T0>(world, entity, pointers);
        }
        ...
        ecs_modified_id(world, entity, ids[0]);
