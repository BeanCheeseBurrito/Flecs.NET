...
public unsafe partial struct ObserverBuilder<T0, T1> : IDisposable, IEquatable<ObserverBuilder<T0, T1>>, IQueryBuilder<ObserverBuilder<T0, T1>, Observer<T0, T1>>
{
    ...
    ///
    public ref ObserverBuilder<T0, T1> Ctx(void* data)
    {
    ...
    public Observer<T0, T1> Run(Action callback)
    {
        return new Observer<T0, T1>(_observerBuilder.Run(callback));
    }
    
    /// <inheritdoc cref="ObserverBuilder.Run(System.Action)"/>
    public Observer<T0, T1> Run(delegate*<void> callback)
    {
        return new Observer<T0, T1>(_observerBuilder.Run(callback));
    }
    ...
    private ref ObserverBuilder<T0, T1> SetCallback<T>(T callback, IntPtr invoker) where T : Delegate
    {
        _observerBuilder.SetCallback(callback, invoker);
        return ref this;
    }

    private ref ObserverBuilder<T0, T1> SetCallback(IntPtr callback, IntPtr invoker)
    {
        _observerBuilder.SetCallback(callback, invoker);
        return ref this;
    }

    private ref ObserverBuilder<T0, T1> SetRun<T>(T callback, IntPtr invoker) where T : Delegate
    {
        _observerBuilder.SetRun(callback, invoker);
        return ref this;
    }

    private ref ObserverBuilder<T0, T1> SetRun(IntPtr callback, IntPtr invoker)
    {
        _observerBuilder.SetRun(callback, invoker);
        return ref this;
    }
