...
public class PipelineBuilder : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput((IncrementalGeneratorPostInitializationContext postContext) =>
        {
            Generator.AddSource(postContext, "PipelineBuilder.QueryBuilder.g.cs", QueryBuilder.GenerateExtensions(Type.PipelineBuilder));

            for (int i = 0; i < Generator.GenericCount; i++)
            {
                Generator.AddSource(postContext, $"PipelineBuilder/T{i + 1}.g.cs", GeneratePipelineBuilder(i));
                Generator.AddSource(postContext, $"PipelineBuilder.QueryBuilder/T{i+ 1}.g.cs", QueryBuilder.GenerateExtensions(Type.PipelineBuilder, i));
            }
        });
    }
